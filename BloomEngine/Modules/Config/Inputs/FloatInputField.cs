...
public sealed class FloatInputField(string name, float value, float minValue, float maxValue, Action<float> onValueChanged, Action onInputChanged, Func<float, float> transformValue, Func<float, bool> validateValue) : InputFieldBase<float>(name, value, onValueChanged, onInputChanged, transformValue, validateValue)
{
